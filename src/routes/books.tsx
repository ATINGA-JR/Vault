...
export const Route = createFileRoute("/books")({
  head: () => ({ meta: [{ title: "Books — Vault" }] }),
  component: () => (<AuthGate><AppShell><BooksPage /></AppShell></AuthGate>),
});
...
function BooksPage() {
  ...
  function toggleReading(id: string) {
    setState((s) => ({
      ...s,
      books: s.books.map((b) => b.id === id ? { ...b, reading: !b.reading, read: false } : b),
    }));
  }
  ...
  function toggleRead(id: string) {
    setState((s) => ({
      ...s,
      books: s.books.map((b) => b.id === id ? { ...b, read: !b.read, reading: false } : b),
    }));
  }
        ...
        <TabsContent value={tab} className="mt-6">
          {filtered.length === 0 ? (
            ...
            <ul className="divide-y divide-border rounded-lg border border-border bg-card">
              {filtered.map((b) => (
                <li key={b.id} className={cn("flex items-center gap-3 px-4 py-3 transition-all", b.reading && !b.read && "bg-primary/[0.03]")}>
                  <button
                    ...
                    className={cn(
                      ...
                      b.reading && !b.read ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40",
                    )}
                  >
                    {b.reading && !b.read ? "▶" : ""}
                  </button>
                  ...
                  <button
                    ...
                    className={cn("grid h-5 w-5 shrink-0 place-items-center rounded border", b.read ? "border-primary bg-primary text-primary-foreground" : "border-border-strong")}
                  >
