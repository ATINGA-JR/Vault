...
function CalendarPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Calendar" title="Your month at a glance."
        action={<Button size="sm" onClick={() => { setAddDate(selected ?? today); setOpenAdd(true); }}>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <div className="rounded-lg border border-border bg-card p-3">
            <div className="grid grid-cols-7 gap-1">
              {DAY_LABEL.map((d) => <div key={d} className="py-2 text-center text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{d}</div>)}
              {grid.map((d) => {
                ...
                return (
                  <button key={iso} onClick={() => openAddFor(iso)}
                    className={cn("group aspect-square min-h-[60px] rounded-md border p-1.5 text-left transition-all",
        ...
        <div>
          ...
          {panelEvents.length === 0 ? (
            <EmptyState icon={<CalIcon className="h-5 w-5" />} title="No events." body={selected ? "Tap this date again to add one." : "Add an event to start."} />
          ) : (
            <ul className="space-y-2">
              {panelEvents.sort((a, b) => (a.date + (a.time ?? "")).localeCompare(b.date + (b.time ?? ""))).map((e) => (
                <li key={e.id} className="group rounded-lg border border-border bg-card p-3">
                  <div className="flex items-start gap-3">
                    ...
                    <button onClick={() => setEditEvent(e)} className="opacity-0 transition-opacity group-hover:opacity-100 text-muted-foreground hover:text-foreground">
      ...
      <EventDialog open={openAdd} onOpenChange={setOpenAdd} initialDate={addDate} />
      <EventDialog open={!!editEvent} onOpenChange={(o) => !o && setEditEvent(null)} editEvent={editEvent ?? undefined} />
    </>
  );
}
...
function buildGrid(cursor: Date) {
  ...
  for (let i = 0; i < startDay; i++) { const d = new Date(first); d.setDate(d.getDate() - (startDay - i)); days.push({ date: d, inMonth: false }); }
  ...
  while (days.length % 7 !== 0) { const last = days[days.length - 1].date; const d = new Date(last); d.setDate(d.getDate() + 1); days.push({ date: d, inMonth: false }); }
  return days;
}

function EventDialog({ open, onOpenChange, initialDate, editEvent }: { open: boolean; onOpenChange: (o: boolean) => void; initialDate?: string; editEvent?: CalendarEvent }) {
  ...
  function save(e: React.FormEvent) {
    ...
    if (!name.trim()) return;
    if (editEvent) {
      setState((s) => ({ ...s, events: s.events.map((x) => x.id === editEvent.id ? { ...x, name: name.trim(), date, time: time || undefined, color } : x) }));
      toast.success("Event updated");
    } else {
      setState((s) => ({ ...s, events: [...s.events, { id: uid(), name: name.trim(), date, time: time || undefined, color, createdAt: new Date().toISOString() }] }));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle className="font-serif text-2xl">{editEvent ? "Edit event" : "New event"}</DialogTitle></DialogHeader>
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button type="submit" className="flex-1">{editEvent ? "Save" : "Add event"}</Button>
            {editEvent && <Button type="button" variant="outline" onClick={remove} className="text-destructive"><Trash2 className="h-4 w-4" /></Button>}
          </div>
        </form>
      </DialogContent>
    </Dialog>
