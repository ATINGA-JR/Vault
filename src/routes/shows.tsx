...
import { Tv, Plus, Trash2, Play } from "lucide-react";
...
export const Route = createFileRoute("/shows")({
  head: () => ({ meta: [{ title: "Shows — Vault" }] }),
  component: () => (<AuthGate><AppShell><ShowsPage /></AppShell></AuthGate>),
});

function ShowsPage() {
  const shows = useStore((s) => s.shows);
  const watched = shows.filter((m) => m.watched).length;
  const watching = shows.filter((m) => m.watching && !m.watched).length;
  const togo = shows.filter((m) => !m.watching && !m.watched).length;
  const [tab, setTab] = useState("all");

  const filtered =
    tab === "all"      ? shows :
    tab === "todo"     ? shows.filter((m) => !m.watching && !m.watched) :
    tab === "watching" ? shows.filter((m) => m.watching && !m.watched) :
                         shows.filter((m) => m.watched);

  function toggleWatching(id: string) {
    setState((s) => ({ ...s, shows: s.shows.map((m) => m.id === id ? { ...m, watching: !m.watching, watched: false } : m) }));
  }
  function toggleWatched(id: string) {
    setState((s) => ({ ...s, shows: s.shows.map((m) => m.id === id ? { ...m, watched: !m.watched, watching: false } : m) }));
  }
  function remove(id: string) {
    setState((s) => ({ ...s, shows: s.shows.filter((m) => m.id !== id) }));
    ...
    <>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value={tab} className="mt-6">
          {filtered.length === 0 ? (
            <EmptyState icon={<Tv className="h-5 w-5" />} title="Nothing here." body="Add a show to get started." />
          ) : (
...
function AddShow() {
  ...
  function submit(e: React.FormEvent) {
    ...
    setState((s) => ({
      ...s,
      shows: [...s.shows, { id: uid(), title: title.trim(), year: year ? parseInt(year) : undefined, watching: false, watched: false, createdAt: new Date().toISOString() }],
    }));
    toast.success("Added");
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)} size="sm"><Plus className="mr-1 h-4 w-4" />Add show</Button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle className="font-serif text-2xl">Add a show</DialogTitle></DialogHeader>
