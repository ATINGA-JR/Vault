...
import {
  LayoutDashboard, ListTodo, BookOpen, Film, Tv, ShoppingCart,
  Settings as SettingsIcon, Bell, Sun, Moon, LogOut, Menu, X,
} from "lucide-react";
...
const NAV = [
  ...
  { to: "/books", label: "Books", icon: BookOpen, key: "books" as const },
  { to: "/movies", label: "Movies", icon: Film, key: "movies" as const },
  { to: "/shows", label: "Shows", icon: Tv, key: "shows" as const },
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="sticky top-0 z-30 flex items-center justify-between border-b border-border bg-background/95 px-4 py-3 backdrop-blur-md md:hidden">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => setMobileOpen(true)} className="rounded-md p-2 hover:bg-accent" aria-label="Open menu">
      ...
      <div className="flex">
        ...
        <div className={cn("fixed inset-0 z-50 md:hidden transition-all duration-300", mobileOpen ? "visible" : "invisible")}>
          <div
            className={cn("absolute inset-0 bg-foreground/40 backdrop-blur-sm transition-opacity duration-300", mobileOpen ? "opacity-100" : "opacity-0")}
          ...
          >
            <button onClick={() => setMobileOpen(false)} className="absolute right-3 top-3 z-10 rounded-md p-1.5 text-sidebar-foreground/60 hover:bg-sidebar-accent" aria-label="Close menu">
              <X className="h-4 w-4" />
            </button>
            {sidebarInner}
          </aside>
